...
def computeTP(t,distS):
    ...
    for j in list:
       ...
       for i in range(len(a.vec.conc)):
            anC.append(2.0*diffL*(SourceC/(math.sqrt(4*math.pi*a.diffConst*j[1])))*math.exp(-(mm[i].Coordinates[0]**2)/(4*a.diffConst*j[1])))
            for k in range(len(a.vec.conc)):
              anC2.append(4.0*diffL**2*(SourceC/(4*numpy.pi*j[1]*numpy.sqrt(a.diffConst*a.diffConst)))*numpy.exp(-(mm[i].Coordinates[0]**2)/(4*a.diffConst*j[1])-(mm[k].Coordinates[0]**2)/(4*a.diffConst*j[1])))
              v2d[i][k] = 4.0*diffL**2*(SourceC/(4*numpy.pi*j[1]*numpy.sqrt(a.diffConst*a.diffConst)))*numpy.exp(-(mm[i].Coordinates[0]**2)/(4*a.diffConst*j[1])-(mm[k].Coordinates[0]**2)/(4*a.diffConst*j[1]))
              firstCell = diffL*(SourceC/(math.sqrt(4*math.pi*a.diffConst*j[1])))*math.exp(-(mm[0].Coordinates[0]**2)/(4*a.diffConst*j[1]))
              firstCell2 = diffL**2*(SourceC/(4*numpy.pi*j[1]*numpy.sqrt(a.diffConst*a.diffConst)))*numpy.exp(-(mm[0].Coordinates[0]**2)/(4*a.diffConst*j[1])-(mm[0].Coordinates[0]**2)/(4*a.diffConst*j[1]))
