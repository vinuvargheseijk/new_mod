...
def makeModel():
    ...
    **b** has a negative feedback onto **a**.
    ...
    e5 = moose.MMenz( '/model/compartment/e5' )
    e6 = moose.MMenz( '/model/compartment/e6' )
    e7 = moose.MMenz( '/model/compartment/e7' )
    e8 = moose.MMenz( '/model/compartment/e8' )
    r1 = moose.Reac( '/model/compartment/r1' )
    rec = moose.Pool( '/model/compartment/rec' )
    kzr1 = moose.Pool( '/model/compartment/kzr1' )
    kzr2 = moose.Pool( '/model/compartment/kzr2' )
    kzr21 = moose.Pool( '/model/compartment/kzr21' )
    kzr2a = moose.Pool( '/model/compartment/kzr2a' )
    ...
    moose.connect( e5, 'sub', kzr2, 'reac')
    moose.connect( e5, 'prd', a, 'reac')
    moose.connect( kzr2a, 'nOut', e5, 'enzDest')
    e4.Km = 0.001
    e4.kcat = 4
    
    
    moose.connect( e6, 'sub', kzr2a, 'reac')
    moose.connect( e6, 'prd', kzr1, 'reac')
    moose.connect( kzr21, 'nOut', e6, 'enzDest')
    e4.Km = 0.001
    e4.kcat = 4
    
    moose.connect( e7, 'sub', kzr2a, 'reac')
    moose.connect( e7, 'prd', kzr1, 'reac')
    moose.connect( a, 'nOut', e7, 'enzDest')
    e4.Km = 0.001
    e4.kcat = 4
    
    moose.connect( e8, 'sub', kzr2, 'reac')
    moose.connect( e8, 'prd', kzr1, 'reac')
    moose.connect( kzr21, 'nOut', e8, 'enzDest')
    e4.Km = 0.001
    e4.kcat = 4
    ...
    assert( dsolve.numPools == 8 )
    a.vec.concInit = [0.1]*num
    kzr1.vec.concInit = [1.5]*num
    kzr2.vec.concInit = [0.5]*num
...
#    a.vec[50].concInit *= 1.2 # slight perturbation at one end.
    ...
    kzr1.vec[10].concInit *=1.2
    kzr2.vec[10].concInit *=1.2
    kzr1.vec[35].concInit *=1.2
    kzr2.vec[35].concInit *=1.2
...
def main():
    ...
    s = moose.element( '/model/compartment/s' )
    rec = moose.element( '/model/compartment/rec' )
    kzr1 = moose.element( '/model/compartment/kzr1' )
    kzr2 = moose.element( '/model/compartment/kzr2' )
    kzr2a = moose.element( '/model/compartment/kzr2a' )
    kzr21 = moose.element( '/model/compartment/kzr21' )
    print moose.showfields(a)
...
#    img = mpimg.imread( 'turingPatternTut.png' )
  ...
  #  imgplot = plt.imshow( img )
    ...
    line5, = ax.plot( pos, a.vec.conc, label='kzr1' )
    line6, = ax.plot( pos, b.vec.conc, label='kzr2' )
    line7, = ax.plot( pos, rec.vec.conc, label='kzr2a')
    line8, = ax.plot( pos, s.vec.conc, label='kzr21')
    ...
    for ti in range( displayInterval, runtime, displayInterval ):
        if ti>150 and ti<158:
          print stimPoint
          if gaussian == True:
             tempRec = computeTP(timePer,distS)
             #for pos in posPer[stimPoint]:
             print pos, 'position'
             rec.vec[posPer[stimPoint]].conc = rec.vec[posPer[stimPoint]].conc+tempRec[0]
             for i in range(posPer[stimPoint]+1,len(a.vec.conc)):
                 rec.vec[i].conc=rec.vec[i].conc+tempRec[i-posPer[stimPoint]]
                 if int(posPer[stimPoint]*2-i)>=0:
                    rec.vec[posPer[stimPoint]+posPer[stimPoint]-i].conc=rec.vec[posPer[stimPoint]+posPer[stimPoint]-i].conc+tempRec[i-posPer[stimPoint]]
        ...
        line5.set_ydata( kzr1.vec.conc )
        line6.set_ydata( kzr2.vec.conc )
        line7.set_ydata( kzr2a.vec.conc)
        line8.set_ydata( kzr21.vec.conc *0.1)
